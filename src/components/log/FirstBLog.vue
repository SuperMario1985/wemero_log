...
export default {
  ...
  methods: {
    ...
    getList() {
      let that = this;
      this.isLoading = true;
      httpService.getFirstBLogList(this.searchInfo, function (response) {
    ...
    showDetail(item) {
      ...
      this.isLoading = true;
      httpService.getFirstBLogDetail(this.logDetail, function (response) {
        that.isLoading = false;
        if (response.success) {
    ...
    downloadLog(item) {
      ...
      if (this.searchInfo.environment === "online") {
        host = "https://firstadmin.wemero.com";
      } else if (this.searchInfo.environment === "pre") {
        host = "https://y-firstadmin.wemero.com";
      } else {
        host = "http://firstadmin.alios.idengyun.com";
        // host = "http://39.105.94.5:8007";
      }
      let url =
        host +
        "/api/firstb/apiLogs/downLogFile?" +
        "log_type=" +
        item.type +
        "&log_date=" +
        item.date;
